...
public class UtilidadesPersonaje {
    ...
    public Map<Region, Personaje> getMasPoderosoPorRegion(List<Personaje> personajes){
        ...
        Map<Region, Personaje> mapapoderosos = new HashMap<>();

        Map<Region, List<Personaje>> personajespor1 = getPersonajesPorRegion(personajes);

        //List<Personaje> listpersona = new ArrayList<>();


        for(Region r : personajespor1.keySet()){
            List<Personaje> listpersona = new ArrayList<>();
            for(Personaje pe : personajes){
                if(pe.getRegion()==r){
                    listpersona.add(pe);
                }
               Personaje poderoso = getMasPoderoso(listpersona);
                mapapoderosos.put(r, poderoso);
            }


    }
        return mapapoderosos;
